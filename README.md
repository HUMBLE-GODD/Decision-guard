...
This merged edition also includes repository-native decision memory: durable `.decisions/` records, two-phase decision logging, dependency/blast-radius analysis, and decision preflight tools for preventing risky regressions.
...
That's what DECISIONGUARD is. Not a retrieval layer. A cognitive architecture.
...
All of this is in DECISIONGUARD. Running. In production. Powering a fleet of agents — each carrying 5,000+ active memories and 1.9 million+ synaptic connections on average — through 6+ months of uninterrupted nightly dream cycles and a 0.00 identity drift score that means the agents wake up the same people they were when they went to sleep.
...
| **Language** | TypeScript (Node 22+) · [Python port](https://github.com/Rezzyman/decisionguard-python) · [Zero-config SQLite version](https://github.com/Rezzyman/decisionguard-lite) |
| **Agents** | Any MCP client (Claude Code, Cursor, Windsurf) · REST API for everything else |

---

## What Makes DECISIONGUARD Different

Most agent memory systems store and retrieve. DECISIONGUARD **learns**.

| Feature | Vector DBs | RAG Pipelines | DECISIONGUARD |
|---------|-----------|---------------|--------|
...
- Ollama for local embeddings (recommended default), or an embedding API key such as VoyageAI

### Install

```bash
git clone https://github.com/Rezzyman/decisionguard.git
cd decisionguard
npm install
cp .env.example .env
...
DECISIONGUARD exposes its tools via the [Model Context Protocol](https://modelcontextprotocol.io/), making it available to any MCP-compatible agent (Claude Code, OpenClaw, custom agents):
...
```json
{
  "mcpServers": {
    "decisionguard": {
      "command": "npx",
      "args": ["tsx", "/path/to/decisionguard/src/mcp/server.ts"],
      "env": {
...
### Run the dashboard and semantic graph demo
...
The dashboard's **Semantic graph** panel reads `/api/v1/graph` for the agent entered in the memory-search card. It renders active memories as category-colored nodes, learned synapses as directional links, and opens a node inspector when a memory is selected. The panel intentionally reflects the database as-is: an empty or one-node graph means that agent has not ingested enough related memories to form connections.
...
```bash
npm run demo:decisions
npm run smoke:api
```

For Groq-backed LLM features, set `DECISIONGUARD_LLM_PROVIDER=groq` and `GROQ_API_KEY`. Embeddings still use Ollama or VoyageAI.
...
| Tool | What It Does |
|------|-------------|
| `decisionguard_init` | Session boot. Loads top memories, system stats, active entities. Call this first. |
| `decisionguard_search` | Hybrid 7-factor search across all memories |
| `decisionguard_recall` | Token-budget-aware retrieval. Ask for 4,000 tokens of context, get exactly that. |
| `decisionguard_ingest` | Store new memory. Chunks, embeds, extracts entities, encodes through hippocampus, forms synapses. |
| `decisionguard_status` | System health and statistics |
| `decisionguard_graph` | Return semantic memory nodes, synaptic links, and graph statistics |
| `decisionguard_dream` | Trigger a dream cycle (full, SWS-only, REM-only, pruning-only) |

### Memory Evolution
| Tool | What It Does |
|------|-------------|
| `decisionguard_reconsolidate` | Update a recalled memory within its labile window |
| `decisionguard_labile` | List memories currently open for modification |

### Skills
| Tool | What It Does |
|------|-------------|
| `decisionguard_procedural_store` | Teach the agent a skill, workflow, or habit |
| `decisionguard_procedural_retrieve` | Find relevant procedures for a given context |

### Self-Awareness (Phases 1-6)
| Tool | What It Does |
|------|-------------|
| `decisionguard_self_check` | Run full cognitive integrity diagnostics |
| `decisionguard_journal` | Log agent state (energy, confidence, concerns) |
| `decisionguard_assess_state` | Infer human's energy/stress/focus from their messages |
| `decisionguard_bg_thread` | Run strategic, operational, or relational background reasoning |
| `decisionguard_synthesize` | Generate novel insights from recent synaptic connections |
| `decisionguard_observe` | Capture and analyze screen state (macOS) |
| `decisionguard_relationship` | Lookup a contact's profile, history, and open items |
| `decisionguard_reason` | Store a structured decision trace for audit |
| `decisionguard_audit` | Weekly reasoning consistency check with bias detection |
| `decisionguard_monologue` | Record inner thoughts (the agent's internal voice) |

---

## Search Scoring

DECISIONGUARD uses a 7-factor hybrid scoring system:
...
## The DECISIONGUARD ecosystem

DECISIONGUARD ships as a small family of compatible projects. Pick the one that matches how you want to integrate:

| Project | Language | Storage | Audience | Install |
|---|---|---|---|---|
| **[decisionguard](https://github.com/Rezzyman/decisionguard)** (this repo) | TypeScript (Node 22+) | PostgreSQL + pgvector | Production agent teams · benchmark-certified retrieval · MCP and REST | `git clone` + Docker Compose |
| **[decisionguard-lite](https://github.com/Rezzyman/decisionguard-lite)** | Python 3.10+ | SQLite (one file) | Individual developers · zero-config · local embeddings · 30-second quickstart | `pip install decisionguard-lite` |
| **[decisionguard-python](https://github.com/Rezzyman/decisionguard-python)** | Python 3.10+ | PostgreSQL + pgvector (shares schema with this repo) | Python agent codebases that need to read and write the same memory store as a TypeScript DECISIONGUARD deployment | `pip install decisionguard-ai` |

**Choose decisionguard** for the full benchmark-certified stack with dream cycles, CA3 pattern completion, reconsolidation, emotional valence, procedural learning, and autonomous cognition threads.

**Choose decisionguard-lite** to get the hybrid-search pattern running in under a minute with zero infrastructure. Graduate to full decisionguard when you need dream cycles or scale past ~10K memories.

**Choose decisionguard-python** when your agent code is already Python and you need first-class read/write access to a DECISIONGUARD memory store. decisionguard-python implements the core subsystems (hippocampus, dream cycle, procedural memory, hybrid search) and is schema-compatible with this flagship repo.
...
Priority areas: additional benchmarks, entity resolution, temporal reasoning queries, additional embedding providers, expanded test coverage, and bringing decisionguard-python to full parity with the TypeScript flagship.
