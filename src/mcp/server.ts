#!/usr/bin/env node
/**
 * DECISIONGUARD V2 MCP Server
 *
 * Exposes DECISIONGUARD memory operations as native tools for Claude Code.
 * Any agent gets: decisionguard_search, decisionguard_recall, decisionguard_init,
 * decisionguard_ingest, decisionguard_dream, decisionguard_status as tool calls.
 *
...
const decisionRoot = () => process.env.DECISIONGUARD_WORKSPACE || process.cwd();

server.tool(
  "decisionguard_decision_query",
  "Search durable architectural decisions and expand results through dependencies and blast radius. Use before making a consequential change.",
  { query: z.string(), top_n: z.number().default(5) },
  async ({ query, top_n }) => ({ content: [{ type: "text" as const, text: JSON.stringify(queryDecisions(loadDecisionRecords(decisionRoot()), query, top_n), null, 2) }] })
);

server.tool(
  "decisionguard_decision_classify",
  "Classify whether a proposed decision should be recorded and how important it is.",
  { description: z.string() },
  async ({ description }) => ({ content: [{ type: "text" as const, text: JSON.stringify(classifyDecision(description), null, 2) }] })
);

server.tool(
  "decisionguard_decision_log",
  "Two-phase decision logger. First call without confirmed=true returns related decisions and a token; second call must provide that token and alternatives for heavy decisions.",
  {
    description: z.string(), confirmed: z.boolean().default(false), token: z.string().optional(),
    weight: z.enum(["heavy", "standard", "deferred", "skip"]).optional(), category: z.string().optional(),
    title: z.string().optional(), why: z.string().optional(), trade_off: z.string().optional(),
    review_trigger: z.string().optional(), alternatives: z.array(z.string()).optional(),
    relationships: z.array(z.object({ type: z.string(), target: z.string() })).optional(),
  },
  async ({ description, confirmed, token, weight, category, title, why, trade_off, review_trigger, alternatives, relationships }) => {
    const result = confirmDecision({ root: decisionRoot(), description, confirmed, token, weight, category, title, why, tradeOff: trade_off, reviewTrigger: review_trigger, alternatives, relationships });
    const text = result.preview ? `${result.message}\n\n${formatDecisionPreview(result.preview)}` : JSON.stringify(result, null, 2);
    return { content: [{ type: "text" as const, text }] };
  }
);

server.tool(
  "decisionguard_decision_chain",
  "Show what a decision depends on and the decisions that would be affected if it changed.",
  { decision_id: z.string() },
  async ({ decision_id }) => {
    const chain = decisionChain(loadDecisionRecords(decisionRoot()), decision_id);
    return { content: [{ type: "text" as const, text: chain ? JSON.stringify(chain, null, 2) : `Decision '${decision_id}' not found.` }] };
  }
);

server.tool(
  "decisionguard_decision_sync",
  "Sync committed decision records into DECISIONGUARD living memory so they participate in hippocampal encoding, graph links, and recall.",
  { agent_id: z.string().default("arlo") },
  async ({ agent_id }) => {
    try {
      const result = await syncDecisionsToMemory(decisionRoot(), agent_id);
      return { content: [{ type: "text" as const, text: `Synced ${result.records} decision records and ${result.chunks} memory chunks into DECISIONGUARD.` }] };
    } catch (error) {
      return { content: [{ type: "text" as const, text: `Decision sync failed: ${error instanceof Error ? error.message : String(error)}` }] };
    }
  }
);
...
server.tool(
  "decisionguard_graph",
  "Return the semantic memory graph for an agent: encoded memory nodes, learned synaptic links, and graph statistics.",
  {
    agent_id: z.string().default("arlo").describe("Agent ID"),
    limit: z.number().min(1).max(500).default(120).describe("Maximum number of memory nodes to return"),
  },
  async ({ agent_id, limit }) => {
    const agentId = await resolveAgent(agent_id);
    const [nodesResult, synapsesResult, totalResult] = await Promise.all([
      db.execute(sql`
        SELECT id, content, source, source_type, semantic_tags, priority, resonance_score, access_count, status
        FROM memory_nodes
      ...
      db.execute(sql`
        ...
        INNER JOIN memory_nodes a ON a.id = ms.memory_a AND a.agent_id = ${agentId} AND a.status = 'active'
        INNER JOIN memory_nodes b ON b.id = ms.memory_b AND b.agent_id = ${agentId} AND b.status = 'active'
        ORDER BY ms.connection_strength DESC
        LIMIT ${limit * 6}
      `),
      db.execute(sql`
        SELECT COUNT(*) AS count FROM memory_synapses ms
        INNER JOIN memory_nodes a ON a.id = ms.memory_a AND a.agent_id = ${agentId}
        INNER JOIN memory_nodes b ON b.id = ms.memory_b AND b.agent_id = ${agentId}
      `),
    ]);
    return {
      content: [{
        ...
        text: JSON.stringify({
          ...
          synapses: synapsesResult.rows,
          stats: { returnedNodes: nodesResult.rows.length, returnedSynapses: synapsesResult.rows.length, totalSynapses: Number((totalResult.rows[0] as { count?: string })?.count || 0) },
        }, null, 2),
      }],
    };
  }
...
// ─── Tool: decisionguard_search ────────────────────────────────
server.tool(
  "decisionguard_search",
  "Search DECISIONGUARD memory using hybrid scoring (semantic + text + recency + resonance + priority). Use this whenever you need to recall information from past conversations, files, transcripts, or any stored memory.",
  {
  ...
  async ({ query, agent_id, limit, verbose }) => {
    ...
    if (resultIds.length > 0) {
      try {
        ...
        console.error("[decisionguard_search] access count update failed (non-fatal):", err);
      }
    }
    ...
    return {
      content: [
        {
          type: "text" as const,
          text: `# DECISIONGUARD Search: "${query}"\nResults: ${results.rows.length}\n\n${formatted || "No results found."}`,
        },
      ],
    };
  }
...
// ─── Tool: decisionguard_recall ────────────────────────────────
server.tool(
  "decisionguard_recall",
...
// ─── Tool: decisionguard_init ──────────────────────────────────
server.tool(
  "decisionguard_init",
  "Initialize a DECISIONGUARD session. Loads the top memories by hybrid score plus system stats. Use this at the start of every session as part of the boot sequence.",
  {
    agent_id: z
      .string()
      .default("arlo")
  ...
  async ({ agent_id }) => {
    ...
    const now = new Date();
    let output = `# DECISIONGUARD V2 Session Context\n`;
    ...
    output += `---\n*DECISIONGUARD V2 ready. Use decisionguard_search for recall, decisionguard_recall for budget-aware context.*`;
...
// ─── Tool: decisionguard_ingest ────────────────────────────────
server.tool(
  "decisionguard_ingest",
  "Store new content into DECISIONGUARD memory. Automatically chunks, embeds, extracts entities, and forms synapses. Use this to save important information, decisions, learnings, or any content that should persist across sessions.",
  {
...
// ─── Tool: decisionguard_ingest_file ───────────────────────────
server.tool(
  "decisionguard_ingest_file",
  "Ingest a file from disk into DECISIONGUARD memory. Reads, chunks, embeds, and stores the file. Use this for ingesting markdown files, logs, transcripts, etc.",
  {
...
// ─── Tool: decisionguard_ingest_corpus ─────────────────────────
server.tool(
  "decisionguard_ingest_corpus",
  "Ingest the entire V1 corpus (all memory files, telegram, limitless, logs, core files) into DECISIONGUARD V2. This is a one-time bulk operation for initial setup. Takes several minutes.",
  {
    agent_id: z
      .string()
      .default("arlo")
...
// ─── Tool: decisionguard_status ────────────────────────────────
server.tool(
  "decisionguard_status",
  "Get DECISIONGUARD system status including memory count, synapse count, resonance stats, and last dream cycle info.",
  {
    agent_id: z
      .string()
      .default("arlo")
  ...
  async ({ agent_id }) => {
    ...
    let output = `# DECISIONGUARD V2 Status\n\n`;
...
// ─── Tool: decisionguard_artifact ──────────────────────────────
server.tool(
  "decisionguard_artifact",
...
// ─── Tool: decisionguard_self_check (Phase 1) ─────────────────
server.tool(
  "decisionguard_self_check",
...
// ─── Tool: decisionguard_journal (Phase 1) ────────────────────
server.tool(
  "decisionguard_journal",
...
// ─── Tool: decisionguard_assess_state (Phase 2) ──────────────
server.tool(
  "decisionguard_assess_state",
...
// ─── Tool: decisionguard_state_history (Phase 2) ─────────────
server.tool(
  "decisionguard_state_history",
...
// ─── Tool: decisionguard_bg_thread (Phase 3) ─────────────────
server.tool(
  "decisionguard_bg_thread",
...
// ─── Tool: decisionguard_synthesize (Phase 3) ────────────────
server.tool(
  "decisionguard_synthesize",
...
// ─── Tool: decisionguard_observe (Phase 4) ───────────────────
server.tool(
  "decisionguard_observe",
...
// ─── Tool: decisionguard_relationship (Phase 5) ──────────────
server.tool(
  "decisionguard_relationship",
...
// ─── Tool: decisionguard_relationships (Phase 5) ─────────────
server.tool(
  "decisionguard_relationships",
...
// ─── Tool: decisionguard_relationship_update (Phase 5) ───────
server.tool(
  "decisionguard_relationship_update",
...
// ─── Tool: decisionguard_reason (Phase 6) ────────────────────
server.tool(
  "decisionguard_reason",
...
// ─── Tool: decisionguard_audit (Phase 6) ─────────────────────
server.tool(
  "decisionguard_audit",
...
// ─── Tool: decisionguard_monologue (Phase 6) ─────────────────
server.tool(
  "decisionguard_monologue",
...
// ─── Tool: decisionguard_reconsolidate ─────────────────────────
server.tool(
  "decisionguard_reconsolidate",
...
// ─── Tool: decisionguard_labile ────────────────────────────────
server.tool(
  "decisionguard_labile",
  "List all currently labile (modifiable) memories. These are memories recalled in the last hour that can be updated via decisionguard_reconsolidate.",
  {
    agent_id: z.string().default("arlo").describe("Agent ID"),
  },
  async ({ agent_id }) => {
    ...
    return {
      content: [{ type: "text" as const, text: `# Labile Memories (${labile.length})\nThese can be updated via decisionguard_reconsolidate:\n\n${formatted}` }],
    };
  }
);

// ─── Tool: decisionguard_skill_store ────────────────────────────
server.tool(
  "decisionguard_skill_store",
...
// ─── Tool: decisionguard_skill_retrieve ────────────────────────
server.tool(
  "decisionguard_skill_retrieve",
...
// ─── Tool: decisionguard_skill_executed ────────────────────────
server.tool(
  "decisionguard_skill_executed",
...
// ─── Tool: decisionguard_skill_refine ──────────────────────────
server.tool(
  "decisionguard_skill_refine",
...
async function main() {
  try {
    ...
    console.error("[decisionguard-mcp] Database init warning:", err);
  }

  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("[decisionguard-mcp] DECISIONGUARD V2 MCP server running");
}

main().catch((err) => {
  console.error("[decisionguard-mcp] Fatal:", err);
  process.exit(1);
});
