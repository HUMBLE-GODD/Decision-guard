...
// Keep decision-only/API smoke tests usable without a database. Database-backed
// DECISIONGUARD features still fail clearly during init/query, while the application
// can start and expose the repo-native decision endpoints.
const DATABASE_URL = process.env.DATABASE_URL || "postgresql://decisionguard:decisionguard@localhost:5432/decisionguard";
const isHostedPostgres = DATABASE_URL.includes("neon.tech") || DATABASE_URL.includes("supabase.com");
...
type DecisionGuardDb = Omit<PostgresJsDatabase<typeof schema>, "execute"> & {
  execute: (query: SQL) => Promise<NeonCompatResult>;
};

const origExecute = rawDb.execute.bind(rawDb);
const patchedDb = rawDb as unknown as DecisionGuardDb;
