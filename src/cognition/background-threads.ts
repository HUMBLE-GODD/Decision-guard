/**
 * DECISIONGUARD V2 — Autonomous Cognition: Background Threads
 *
 * Three deterministic reasoning threads that query DECISIONGUARD data:
...
export async function runOperationalThread(agentId: number): Promise<ThreadResult> {
  ...
  if (recentDiags.length === 0) {
    actions.push("No self-diagnostics found. Run `decisionguard self-check` to establish baseline.");
  } else {
