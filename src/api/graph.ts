...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    // These reads are independent after resolving the agent. Run them together so
    // a hosted Postgres round-trip does not make the dashboard feel blocked.
    const synapseLimit = Math.min(limit * 6, 120000); // ~6 synapses per node avg
    const [nodesResult, activeCountResult, archivedResult, totalSynapsesResult, synapsesResult, dreamResult, entitiesResult, sourceResult, diagResult] = await Promise.all([
