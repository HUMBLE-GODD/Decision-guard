/**
 * DECISIONGUARD V2 — Meta-Cognition: Weekly Audit
 *
...
export async function runWeeklyAudit(agentId: number, period?: string): Promise<AuditResult> {
  ...
  const workspaceDir = process.env.DECISIONGUARD_WORKSPACE || process.cwd();
  try {
  ...
  // so they surface in future decisionguard_init and influence future decisions.
  if (biasIndicators.length > 0 || patterns.length > 0) {
    ...
    await db.insert(schema.cognitiveArtifacts).values({
      ...
      // P1 priority equivalent resonance — ensures this surfaces in future decisionguard_init
      resonanceScore: 8.0,
    });
