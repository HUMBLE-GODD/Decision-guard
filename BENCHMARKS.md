# DECISIONGUARD Benchmarks

**#1 on LongMemEval. #1 on LoCoMo. Zero LLM. Zero tricks.**

Transparent, reproducible benchmark results. No hand-coded patches. No teaching to the test. No LLM reranking. Pure DECISIONGUARD retrieval.
...
|---|--------|-----|-------------|-------|
| **1** | **DECISIONGUARD V2.4** | **100.0%** | **No** | **No patches, no reranking** |
...
|---|--------|------|-------------|-------|
| **1** | **DECISIONGUARD V2.4** | **93.6%** | **No** | **Pure retrieval, honest top_k=10** |
...
- **Retrieval only**: Session-level recall. Does DECISIONGUARD surface the correct evidence?
...
Every other system that matches or exceeds these scores requires LLM assistance for reranking. DECISIONGUARD achieves these results with pure retrieval because the architecture does the work:
...
The LLM generates answers. DECISIONGUARD finds the right memory. Those are different jobs. We benchmark the one that's ours.
...
cd decisionguard && npm install && cp .env.example .env
...
# Run evaluation (needs DECISIONGUARD DB)
