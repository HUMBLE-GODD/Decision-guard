...
DECISIONGUARD's architecture maps to established findings in computational neuroscience and cognitive psychology. This document expands the inline citations in the main README and groups them by the subsystem they inform.
...
Maps to: DECISIONGUARD's percentile-based pruning thresholds (P5 delete, P15 archive) computed from the agent's actual resonance distribution, not hardcoded cutoffs.
...
> A modern high-fidelity replication of Ebbinghaus's original data and curve fitting. Confirms the exponential form and quantifies stability parameters that DECISIONGUARD uses as defaults.
...
> The foundational valence × arousal coordinate system that the DECISIONGUARD valence representation extends to six dimensions.
...
Maps to: The architectural parallel that makes CA3 pattern completion the "attention layer" of DECISIONGUARD's memory graph.
...
Both benchmarks are standard in the agent-memory literature and are run by DECISIONGUARD without LLM reranking or hand-coded patches. See [BENCHMARKS.md](BENCHMARKS.md) for methodology and the full leaderboard.

---

## A note on the mapping from neuroscience to engineering

Every subsystem above is based on mechanisms that have been independently replicated across laboratories and modeled computationally. DECISIONGUARD does not claim its implementation *is* a brain — it claims that the engineering problem "how do you give an agent real memory" has a solved analogue in biology, and that borrowing the solved solution beats inventing a new one from scratch.
...
<!-- decisionguard-canary-2026-v2.4.1-a7f3c9e1 -->
