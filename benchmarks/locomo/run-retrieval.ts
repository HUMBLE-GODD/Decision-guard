/**
 * LoCoMo Retrieval-Only Benchmark for DECISIONGUARD
 *
 * Same methodology as our LongMemEval 500/500 run:
 * pure retrieval, no LLM, no tricks.
 *
 * For each question, checks if DECISIONGUARD retrieves chunks from
 * the correct evidence session(s). Scores Recall@K and MRR.
 *
 * This isolates DECISIONGUARD's contribution (finding the right memory)
...
async function main() {
  console.log("============================================");
  console.log("  DECISIONGUARD V2.4 -- LoCoMo Retrieval Benchmark");
  ...
  console.log(`DECISIONGUARD V2.4 (no LLM): ${(overall.recallAt10 * 100).toFixed(1)}%`);
