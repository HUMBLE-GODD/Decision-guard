...
```bash
# Clone and install DECISIONGUARD
git clone https://github.com/aterna-ai/decisionguard.git
cd decisionguard
npm install
cp .env.example .env
...
benchmarks/cogbench/
  ...
  client.ts             # Extended DECISIONGUARD benchmark client
  generate.ts           # Dataset generator (no DB needed)
  run.ts                # Evaluation harness (needs DECISIONGUARD DB)
  README.md             # This file
  tasks/
...
@inproceedings{juarez2026cogbench,
  ...
  year      = {2026},
  url       = {https://github.com/aterna-ai/decisionguard}
}
```

## License

Apache 2.0 — same as DECISIONGUARD.
