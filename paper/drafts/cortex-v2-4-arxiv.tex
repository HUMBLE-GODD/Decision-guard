...
\hypersetup{
  ...
  pdftitle={DECISIONGUARD: A Neuroscience-Grounded Memory Architecture Leading Retrieval Benchmarks Without LLM Rerankers},
...
\title{%
  DECISIONGUARD: A Neuroscience-Grounded Memory Architecture \\
...
\author{%
  ...
  \url{https://github.com/Rezzyman/decisionguard}
}

\date{Technical note \textperiodcentered{} DECISIONGUARD v2.4.1 \textperiodcentered{} April 2026}
...
\begin{itemize}
  \item \textbf{\texttt{decisionguard}} (TypeScript) --- the reference flagship,
        backed by PostgreSQL + pgvector + Drizzle. Full hippocampal
        stack, dream cycle, reconsolidation, valence, procedural memory,
        MCP server.
  \item \textbf{\texttt{decisionguard-python}} (\texttt{pip install decisionguard-ai})
  ...
  \item \textbf{\texttt{decisionguard-lite}} (\texttt{pip install decisionguard-lite})
...
families are disclosed. Where retrieval R@K is published, DECISIONGUARD v2.4
leads:
...
\midrule
\textbf{DECISIONGUARD v2.4}                                 & \textbf{93.7}            & \textbf{No LLM}    \\
...
\begin{itemize}
  ...
  \item Embedding calls are replayable via a local cache when
        \texttt{DECISIONGUARD\_EMBED\_CACHE=1} is set; benchmarks disable the
...
\begin{itemize}
  ...
  \item \textbf{\texttt{decisionguard-lite}} is documented via integration
