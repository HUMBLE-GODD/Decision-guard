# DECISIONGUARD: A Neuroscience-Grounded Memory Architecture Leading Retrieval Benchmarks Without LLM Rerankers

**Atanasio Juarez**
ATERNA AI — Denver, CO
ajuarez@aterna.ai

*Technical note · DECISIONGUARD v2.4.1 · 2026-04-20*

---

## Abstract

We present DECISIONGUARD, a production memory substrate for agentic AI
...
canonical long-term memory benchmarks, DECISIONGUARD v2.4 achieves
...
DECISIONGUARD is a memory substrate for agentic AI modeled on the mammalian
...
- **`decisionguard`** (TypeScript) — the reference flagship, backed by
  PostgreSQL + pgvector + Drizzle. Full hippocampal stack, dream cycle,
  reconsolidation, valence, procedural memory, MCP server.
- **`decisionguard-python`** (`pip install decisionguard-ai`) — a Python port of the
...
- **`decisionguard-lite`** (`pip install decisionguard-lite`) — a single-file SQLite
...
The DECISIONGUARD v2.4 hybrid score is a linear combination of six factors,
...
DECISIONGUARD v2.4 leads:

| System                                 | LoCoMo R@10 | Retrieval path       |
|----------------------------------------|-------------|----------------------|
| **DECISIONGUARD v2.4**                        | **93.7**    | **No LLM**           |
...
top published scores cluster at or just below DECISIONGUARD's accuracy with
...
- Embedding calls are replayable via a local cache when
  `DECISIONGUARD_EMBED_CACHE=1` is set; benchmarks disable the cache by
...
The full benchmark harness ships with the public DECISIONGUARD release:
...
- **`decisionguard-lite`** is documented via integration tests only. It is
